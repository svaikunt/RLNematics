module Analysis
    ...
    function GetDefectPosList(agentHandlerArray)
        posPlusArray = []
        posMinArray = []
        for t in 1:length(agentHandlerArray)
            if length(agentHandlerArray[t].PlusDefects) == 1
                push!(posPlusArray, deepcopy(agentHandlerArray[t].PlusDefects[1].Position))
            else 
                push!(posPlusArray, [0.0, 0.0])
            end
            if length(agentHandlerArray[t].MinusDefects) == 1
                push!(posMinArray, deepcopy(agentHandlerArray[t].MinusDefects[1].Position))
            else 
                push!(posMinArray, [0.0, 0.0])
            end
        end 
        return (posPlusArray, posMinArray)
    ...
    function GetTrainingPositionResults(d)
        targetAgentHandlerArray = deepcopy(d["targetAgentHandlerArray"]);
        agentHandlerArrayList = deepcopy(d["agentHandlerArrayList"]);
        (posPlusArrayTarget, posMinArrayTarget) = GetDefectPosList(targetAgentHandlerArray)
        posPlusListTrain = []
        posMinListTrain = []
        for trial in 1:length(agentHandlerArrayList)
            (posPlusArray, posMinArray) = GetDefectPosList(agentHandlerArrayList[trial])
            push!(posPlusListTrain, deepcopy(posPlusArray))
            push!(posMinListTrain, deepcopy(posMinArray))
        end 
        return (posPlusArrayTarget, posMinArrayTarget, posPlusListTrain, posMinListTrain)
    end

    function GetTargetOrientation(d)
        targetAgentHandlerArray = deepcopy(d["targetAgentHandlerArray"]);
        orArray = []
        for t in 1:length(targetAgentHandlerArray)
            push!(orArray, targetAgentHandlerArray[t].MinusDefects[1].Orientation)
        end 
        return orArray
    end
    ...
    function GetHorSeparationTrajectory(d)
        ex = d["ex"]
        sepList = []
        hookResults = ex.hook.episodeList
        for ep in 1:length(hookResults)
            ...
            for t in 1:length(hookResults[ep].agentHandlerList)
                if (length(hookResults[ep].agentHandlerList[t].PlusDefects) == 1) && (length(hookResults[ep].agentHandlerList[t].MinusDefects) == 1)
                    push!(epSeps, hookResults[ep].agentHandlerList[t].PlusDefects[1].Position[1] - hookResults[ep].agentHandlerList[t].MinusDefects[1].Position[1])
                end
            end
            push!(sepList, epSeps)
        end
