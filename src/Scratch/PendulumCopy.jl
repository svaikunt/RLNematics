...
mutable struct PendulumEnvCopy{A,T,R<:AbstractRNG} <: AbstractEnv
...
Random.seed!(env::PendulumEnvCopy, seed) = Random.seed!(env.rng, seed)

pendulum_observation(s) = [cos(s[1]), sin(s[1]), s[2]]
angle_normalize(x) = Base.mod((x + Base.π), (2 * Base.π)) - Base.π

RLBase.action_space(env::PendulumEnvCopy) = env.action_space
RLBase.state_space(env::PendulumEnvCopy) = env.observation_space
RLBase.reward(env::PendulumEnvCopy) = env.reward
RLBase.is_terminated(env::PendulumEnvCopy) = env.done
RLBase.state(env::PendulumEnvCopy) = pendulum_observation(env.state)

function RLBase.reset!(env::PendulumEnvCopy{A,T}) where {A,T}
...
function (env::PendulumEnvCopy)(a::Union{Int, AbstractFloat})
    @assert a in env.action_space
    env.action = torque(env, a)
    _step!(env, env.action)
end

function _step!(env::PendulumEnvCopy, a)
    env.t += 1
    th, thdot = env.state
...
function torque(env::PendulumEnvCopy{<:Base.OneTo}, a::Int)
    return (4 / (env.n_actions - 1)) * (a - (env.n_actions - 1) / 2 - 1)
end

torque(env::PendulumEnvCopy{<:ClosedInterval}, a::AbstractFloat) = a
