export SpeakerListenerEnvCopy

mutable struct SpeakerListenerEnvCopy{T<:Vector{Float64}} <: AbstractEnv
    target::T
    content::T
    player_vel::T
    player_pos::T
...
"""
function SpeakerListenerEnvCopy(;
    N::Int = 3,
    stop = 0.01,
    damping = 0.25,
    max_accel = 0.5,
...
function RLBase.reset!(env::SpeakerListenerEnvCopy)
    env.init_step = 0
    env.play_step = 0
...
RLBase.is_terminated(env::SpeakerListenerEnvCopy) = (reward(env) > - env.ϵ) || (env.play_step > env.max_steps)
RLBase.players(::SpeakerListenerEnvCopy) = (:Speaker, :Listener, CHANCE_PLAYER)

RLBase.state(env::SpeakerListenerEnvCopy, ::Observation{Any}, players::Tuple) = Dict(p => state(env, p) for p in players)

RLBase.state(env::SpeakerListenerEnvCopy, ::Observation{Any}, player::Symbol) = 
...
RLBase.state(env::SpeakerListenerEnvCopy, ::Observation{Any}, ::ChancePlayer) = vcat(env.landmarks_pos, [env.player_pos])

RLBase.state_space(env::SpeakerListenerEnvCopy, ::Observation{Any}, players::Tuple) = 
    Space(Dict(player => state_space(env, player) for player in players))

RLBase.state_space(env::SpeakerListenerEnvCopy, ::Observation{Any}, player::Symbol) = 
    if player == :Speaker
        # env.target
...
RLBase.state_space(env::SpeakerListenerEnvCopy, ::Observation{Any}, ::ChancePlayer) = 
    Space(
        vcat(
            # landmarks' positions
...
RLBase.action_space(env::SpeakerListenerEnvCopy, players::Tuple) = 
        Space(Dict(p => action_space(env, p) for p in players))

RLBase.action_space(env::SpeakerListenerEnvCopy, player::Symbol) = 
    if player == :Speaker
...
function RLBase.action_space(env::SpeakerListenerEnvCopy, ::ChancePlayer)
...
function (env::SpeakerListenerEnvCopy)(action, ::ChancePlayer)
    env.init_step += 1
...
function (env::SpeakerListenerEnvCopy)(actions::Dict, players::Tuple)
...
function (env::SpeakerListenerEnvCopy)(action::Vector, player::Symbol)
    if player == :Speaker
...
RLBase.reward(::SpeakerListenerEnvCopy, ::ChancePlayer) = -Inf

function RLBase.reward(env::SpeakerListenerEnvCopy, p)
    if sum(env.target) == 1
...
RLBase.current_player(env::SpeakerListenerEnvCopy) = 
...
RLBase.NumAgentStyle(::SpeakerListenerEnvCopy) = MultiAgent(2)
RLBase.DynamicStyle(::SpeakerListenerEnvCopy) = SIMULTANEOUS
RLBase.ActionStyle(::SpeakerListenerEnvCopy) = MINIMAL_ACTION_SET
RLBase.InformationStyle(::SpeakerListenerEnvCopy) = IMPERFECT_INFORMATION
RLBase.ChanceStyle(::SpeakerListenerEnvCopy) = EXPLICIT_STOCHASTIC