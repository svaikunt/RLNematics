...
- `rng::AbstractRNG`.
"""
mutable struct MADDPGManagerAlt <: AbstractPolicy
...
function (π::MADDPGManagerAlt)(env::AbstractEnv)
...
function (π::MADDPGManagerAlt)(stage::Union{PreEpisodeStage, PostActStage}, env::AbstractEnv)
...
function (π::MADDPGManagerAlt)(stage::PreActStage, env::AbstractEnv, actions)
...
function (π::MADDPGManagerAlt)(stage::PostEpisodeStage, env::AbstractEnv)
...
# update policy
function RLBase.update!(π::MADDPGManagerAlt, env::ToyEnv)
    π.update_step += 1
    ...
    for (player, agent) in π.agents
        ...
        # println(" ")
        # println(player)
        # println(mean(A(batches[player][:state]) .^ 2))

        if player == :PlusDefect
            gs2 = gradient(Flux.params(A)) do
                loss = -mean(vec(C(vcat(s, 
                vcat(
                    π.agents[:MinusDefect].policy.policy.behavior_actor(batches[:MinusDefect][:next_state]),
                    A(batches[:PlusDefect][:next_state]) 
                    )
            ...
            gs2 = gradient(Flux.params(A)) do
                loss = -mean(vec(C(vcat(s, 
                vcat(
                    A(batches[:MinusDefect][:next_state]),
                    π.agents[:PlusDefect].policy.policy.behavior_actor(batches[:PlusDefect][:next_state])
                    )
                )))) #+ mean(A(batches[player][:state]) .^ 2) * 1e-3
                ignore() do
                   p.actor_loss = loss
        ...
        #         loss = mean(vec(C(vcat(s, 
        #         vcat(
        #             π.agents[:Listener].policy.policy.behavior_actor(batches[:Listener][:next_state]),
        #             A(batches[:Speaker][:next_state]) 
        #             )
        #         )))) #+ mean(A(batches[player][:state]) .^ 2) * 1e-3

        #         #loss = mean(A(batches[player][:state]) .^ 2) * 1e-3
        ...
        #         loss = mean(vec(C(vcat(s, 
        #         vcat(
        #             A(batches[:Listener][:next_state]),
        #             π.agents[:Speaker].policy.policy.behavior_actor(batches[:Speaker][:next_state])
        #             )
        #         )))) #+ mean(A(batches[player][:state]) .^ 2) * 1e-3
